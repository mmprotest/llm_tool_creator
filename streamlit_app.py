...
if run_button:
    if not goal.strip():
        ...
        try:
            ...
            orchestrator: Optional[AgentOrchestrator] = st.session_state.get("_orchestrator")
            if orchestrator is None or st.session_state.get("_agent_signature") != signature:
                try:
                    orchestrator = AgentOrchestrator(agent_config)
                except RuntimeError as exc:
                    st.session_state["_orchestrator"] = None
                    st.session_state["_agent_signature"] = None
